...
def bronze_teams():
    try:
        ...
        for season in seasons:
            ...
            season_stats_opp = leaguedashteamstats.LeagueDashTeamStats(season=season, measure_type_detailed_defense='Advanced')
            df_season = pd.DataFrame(season_stats.get_data_frames()[0])
            df_season_adv = pd.DataFrame(season_stats_opp.get_data_frames()[0])
            df_season_adv = df_season_adv[['TEAM_ID', 'POSS', 'PACE']]
